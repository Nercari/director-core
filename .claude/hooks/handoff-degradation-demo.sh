#!/usr/bin/env bash
# Operator-visible behavior check: handoff validation stops passing when it
# cannot validate. Ticket #48.
#
...
# not have check-jsonschema — which is the machine this is written on, and the
# reason the fail-open survived as long as it did.
stub_dir() {
  local mode="$1" dir
...
# labelled absent — and the fail-open being fixed lived only in the other branch,
# so the whole point of those cases would be lost while they still printed green.
# The directory holding the real binary is removed from PATH instead of trusting
# the label.
absent_path() {
...
#       <handoff-json|OMIT> [drop-schema]
check() {
  local expected="$1" label="$2" validator="$3" inflight="$4" handoff="$5" drop="${6:-}"
...
echo "handoff validation degradation — behavior check"
echo

echo "--- validator absent: the gate must still be a gate"
# Accepted before this change. jq empty passed and nothing else looked.
...
check ALLOWED "valid handoff" absent yes "$VALID"
...
echo "handoff degradation demo: every verdict as specified."
exit 0
