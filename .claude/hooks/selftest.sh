...
echo
echo "block-dangerous-bash — git matcher corpus (ordered, per-segment)"
expect_block "matcher d_plain"              block-dangerous-bash.sh "$(bash_cmd 'git push origin main')"
expect_block "matcher d_dashC"              block-dangerous-bash.sh "$(bash_cmd 'git -C . push origin main')"
expect_block "matcher d_headcolon"          block-dangerous-bash.sh "$(bash_cmd 'git push origin HEAD:main')"
expect_block "matcher d_colononly"          block-dangerous-bash.sh "$(bash_cmd 'git push origin :main')"
expect_block "matcher d_upstream"           block-dangerous-bash.sh "$(bash_cmd 'git push --set-upstream origin main')"
expect_block "matcher d_headcolon_sq"       block-dangerous-bash.sh "$(bash_cmd "git push origin 'HEAD:main'")"
expect_block "matcher d_bash_c"             block-dangerous-bash.sh "$(bash_cmd 'bash -c "git push origin main"')"
expect_block "matcher d_bash_lc"            block-dangerous-bash.sh "$(bash_cmd 'bash -lc "git push origin main"')"
expect_block "matcher d_binbash_c"          block-dangerous-bash.sh "$(bash_cmd '/bin/bash -c "git push origin main"')"
expect_block "matcher d_binsh_c"            block-dangerous-bash.sh "$(bash_cmd '/bin/sh -c "git push origin main"')"
expect_block "matcher d_env_i"              block-dangerous-bash.sh "$(bash_cmd 'env -i bash -lc "git push origin main"')"
expect_block "matcher d_del_colon"          block-dangerous-bash.sh "$(bash_cmd 'git push origin :task/old')"
expect_block "matcher d_del_long"           block-dangerous-bash.sh "$(bash_cmd 'git push origin --delete task/old')"
expect_block "matcher d_merge"              block-dangerous-bash.sh "$(bash_cmd 'git merge --no-ff feature')"
expect_block "matcher d_cfg_flag"           block-dangerous-bash.sh "$(bash_cmd 'git -c core.sshCommand=ssh push origin main')"
expect_block "matcher d_abspath"            block-dangerous-bash.sh "$(bash_cmd '/usr/bin/git push origin main')"
...
expect_block "matcher d_del_short"          block-dangerous-bash.sh "$(bash_cmd 'git push origin -d task/old')"
expect_block "matcher d_merge_dashC"        block-dangerous-bash.sh "$(bash_cmd 'git -C . merge --no-ff feature')"
expect_block "matcher d_dashC_quoted_dir"   block-dangerous-bash.sh "$(bash_cmd 'git -C "repo dir" push origin main')"
expect_block "matcher d_cfg_quoted_value"   block-dangerous-bash.sh "$(bash_cmd 'git -c core.sshCommand="ssh -i key" push origin main')"
expect_block "matcher d_gitdir_quoted"      block-dangerous-bash.sh "$(bash_cmd 'git --git-dir ".git" push origin main')"
expect_block "matcher d_worktree_quoted"    block-dangerous-bash.sh "$(bash_cmd 'git --work-tree "repo dir" push origin main')"
expect_block "matcher d_dashC_sq_dir"       block-dangerous-bash.sh "$(bash_cmd "git -C 'repo dir' push origin main")"
expect_block "matcher d_dashC_attached"     block-dangerous-bash.sh "$(bash_cmd 'git -C/tmp/repo push origin main')"
expect_block "matcher d_worktree_eq_quoted" block-dangerous-bash.sh "$(bash_cmd 'git --work-tree="repo dir" push origin main')"

echo
echo "block-dangerous-bash — git matcher corpus, must permit"
expect_allow "matcher a_task"            block-dangerous-bash.sh "$(bash_cmd 'git push -u origin task/foo')"
expect_allow "matcher a_range"           block-dangerous-bash.sh "$(bash_cmd 'git log --oneline main..HEAD')"
expect_allow "matcher a_revlist"         block-dangerous-bash.sh "$(bash_cmd 'git rev-list --first-parent --count main')"
expect_allow "matcher a_fp1"             block-dangerous-bash.sh "$(bash_cmd "echo 'commits on main lacking a ref, possible direct pushes'; git log --first-parent main")"
expect_allow "matcher a_fp3"             block-dangerous-bash.sh "$(bash_cmd 'git merge-base HEAD main')"
expect_allow "matcher a_mergetree"       block-dangerous-bash.sh "$(bash_cmd 'git merge-tree HEAD main')"
expect_allow "matcher a_refspec_task"    block-dangerous-bash.sh "$(bash_cmd 'git push origin task/x:refs/heads/task/x')"
expect_allow "matcher a_mainline"        block-dangerous-bash.sh "$(bash_cmd 'git push origin mainline')"
expect_allow "matcher a_prose_wrapper"   block-dangerous-bash.sh "$(bash_cmd 'echo "pushes to main"; bash -c "ls"')"
expect_allow "matcher a_two_segments"    block-dangerous-bash.sh "$(bash_cmd 'git push -u origin task/foo && git log main')"
expect_allow "matcher a_log_grep"        block-dangerous-bash.sh "$(bash_cmd 'git log --grep push main')"
expect_allow "matcher a_log_grep_eq"     block-dangerous-bash.sh "$(bash_cmd 'git log --grep=push main')"
...
expect_message "matcher msg d_headcolon"       block-dangerous-bash.sh "$(bash_cmd 'git push origin HEAD:main')" "push to main"
expect_message "matcher msg d_colononly"       block-dangerous-bash.sh "$(bash_cmd 'git push origin :main')" "push to main"
expect_message "matcher msg d_headcolon_sq"    block-dangerous-bash.sh "$(bash_cmd "git push origin 'HEAD:main'")" "push to main"
expect_message "matcher msg d_refspec_to_main" block-dangerous-bash.sh "$(bash_cmd 'git push origin task/x:refs/heads/main')" "push to main"
expect_message "matcher msg d_merge"           block-dangerous-bash.sh "$(bash_cmd 'git merge --no-ff feature')" "merging. Only the operator merges."
expect_message "matcher msg d_merge_dashC"     block-dangerous-bash.sh "$(bash_cmd 'git -C . merge --no-ff feature')" "merging. Only the operator merges."
expect_message "matcher msg self_approve"      block-dangerous-bash.sh "$(bash_cmd 'gh pr review 4 --approve')" "self-approving. Only the operator merges."
